...
export const getCategories = asyncHandler(async (req, res) => {
  ...
  const categories = await categoryService.getCategories(workspace, req.user._id, req.query);
...
export const archiveCategory = asyncHandler(async (req, res) => {
  const category = await categoryService.archiveCategory(req.params.id, req.user._id);
...
export const cloneDefaultCategories = asyncHandler(async (req, res) => {
  const { workspace } = req.validatedData?.body ?? req.body;

  if (!workspace) {
    throw new ApiError(400, "Workspace ID is required.");
  }

  const categories = await categoryService.cloneDefaultCategories(workspace, req.user._id);
